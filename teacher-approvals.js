...
function render(){
 $("profileList").innerHTML=(data.profiles||[]).map(p=>`<div class="ops-item"><div class="ops-item-main"><strong>${p.display_name}</strong><small>${p.headline_en||""} • ${p.staff_active?"Active":"Suspended"}</small></div><button class="ops-btn gold small" onclick="approveProfile('${p.teacher_id}')">Approve & Publish</button></div>`).join("")||`<p class="ops-muted">—</p>`;
